...
class TestHandleEvent:
    ...
    async def test_no_session_not_mention_bot_ignored(self):
        ...
        with patch("src.handler.handle_command") as mock_cmd, \
             patch("src.handler.handle_message") as mock_msg:
            await handle_event(
    ...
    async def test_no_session_mention_bot_plain_text_calls_handle_message(self):
        ...
        with patch("src.handler.handle_message", new_callable=AsyncMock) as mock_msg, \
             patch("src.handler.handle_command", new_callable=AsyncMock) as mock_cmd:
            await handle_event(
                event, feishu, FakeConfig(), FakeAgentManager(),
                FakeSessionManager(), {}, None,
            )
    ...
    async def test_existing_session_command_no_mention_needed(self):
        ...
        with patch("src.handler.handle_command", new_callable=AsyncMock) as mock_cmd, \
             patch("src.handler.handle_message", new_callable=AsyncMock) as mock_msg:
            await handle_event(
                event, feishu, FakeConfig(), FakeAgentManager(),
                sm, {}, None,
            )
